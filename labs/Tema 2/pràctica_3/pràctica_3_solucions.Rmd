...
```{r}
plot(ecdf(diabetes$bmi), col='blue')
lines(c(min(diabetes$bmi), max(diabetes$bmi)), c(0, 1), col='red')
legend(min(diabetes$bmi), 1, c("\\hat{F}_N", 'uniforme'), col = c('blue', 'red'), lty = 1)
```
...
```{r}
plot(ecdf(subset(diabetes$age, diabetes$prog > 140)), 
     col='blue', main="f.d.c.e. de l'edat dels pacients", lty=1, do.points=FALSE, verticals=TRUE, xlab='edat')
lines(ecdf(subset(diabetes$age, diabetes$prog < 140)), col='red', lty=1, do.points=FALSE, verticals=TRUE)
legend(10, 1, c("prog > 140", "prog < 140"), col = c('blue', 'red'), lty = 1)
```
...
```{r}
df = data.frame(norm=rnorm(1000, 0, 2))
df$exp = rexp(1000, 2)
df$poisson = rpois(1000, 10)
df$cauchy = rcauchy(1000, 0, 1)
...
```{r}
plot(diabetes$ldl, diabetes$tc)
plot(diabetes$hdl, diabetes$tch)
```
